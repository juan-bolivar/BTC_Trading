...
class StrategyLearner(object):   
    def __init__(self, verbose=False, impact=0.005 , commission=9.95 , start=10000):
        self.verbose = verbose
        self.impact = impact
        self.commission = commission
        self.sv      = start

    def calculate_reward(self, holdings_actions_1 , action ,key , acc=0):
        prices_all              = self.prices_all
        holdings_actions        = {0:1,1:0.75,2:0.5,3:0.25,4:0,5:-0.25,6:-0.5,7:-0.75,8:-1}[action]
        #{0:1,1:0.5,2:0,3:-0.5,4:-1}[action]l
        ...
        reward = (holdings_diff + acc)/(self.sv)*100
        return (reward,holdings_actions,holdings_diff + acc)
    ...
    def addEvidence(self, symbol = "IBM", \
        ...
        ### PENDIENTE CAMBIAR PARA QUE LEA INDEPENDIENTEMENTE DE LA BASE DE DATOS#####
        ...
        max_iter          = 50    # Iteraciones Maximas para el  aprendizaje
        ...
        for e in range(max_iter):
            ...
            for key,row in states.iloc[1:].iterrows():
                ...
                (reward,holdings_actions,acc) = self.calculate_reward( holdings_actions_1 = holdings_actions , action = action ,key= key, acc=acc)
        ...
        trades['Order']  = trades['Shares'].to_frame().applymap(lambda x : {-2:'SELL',-1.75:'SELL',-1.5:'SELL',-1.25:'SELL',-1:'SELL',-0.75:'SELL',-0.5:'SELL',-0.25:'SELL',0:0,
                                                                             2:'BUY' , 1.75:'BUY' , 1.5:'BUY' , 1.25:'BUY' , 1:'BUY' , 0.75:'BUY' , 0.5:'BUY' , 0.25:'BUY'}[x])
        #trades['Shares'].to_frame().applymap(lambda x : {-2:'SELL',-1.5:'SELL',-1:'SELL',-0.5:'SELL',0:0,0.5:"BUY",1:"BUY",1.5:"BUY",2:"BUY"}[x])
    ...
    def testPolicy(self, symbol = "IBM", \
        ...
        for key,state in states.iterrows():
            ...
            holdings.loc[key] = {0:-1 , 1:-0.75 , 2:-0.5 ,3:-0.25 , 4:0, 5:0.25 , 6:0.5 , 7:0.75, 8:1}[action]
        previous_days = 13
        trades = pd.DataFrame(data = actions_df.iloc[:,-1],index = actions_df.index).diff().shift(-1).fillna(0)
